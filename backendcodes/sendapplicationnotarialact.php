...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['document'])) {
    ...
    $upload_dir = '../adminsection/notarialact/';
    ...
    if (move_uploaded_file($_FILES['document']['tmp_name'], $target_file)) {
        ...
        $sql = "INSERT INTO applicationnotarialact (full_name, national_id, email, phone, act_type, attachment, service_name, ai_forgery_score, ai_verdict, application_date, expected_feedback_date) 
                VALUES ('$full_name', '$national_id', '$email', '$phone', '$act_type', '$target_file', '$service_name', '$forgery_score', '$verdict', '$application_date', '$expected_feedback_date')";

        if (mysqli_query($conn, $sql)) {
            header("Location: ../userdashboard.php?msg=Notarial act submitted for AI forensic screening");
        } else {
