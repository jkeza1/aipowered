<?php
// backendcodes/sendapplicationcourtjudgment.php
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['document'])) {
    ...
    // If the modal had email/phone, we could use them here too:
    ...
    $upload_dir = '../adminsection/courtjudgment/';
    ...
    if (move_uploaded_file($_FILES['document']['tmp_name'], $target_file)) {
        ...
        $sql = "INSERT INTO applicationcourtjudgment (full_name, national_id, email, phone, case_number, ruling_year, attachment, service_name, ai_forgery_score, ai_verdict, application_date, expected_feedback_date) 
                VALUES ('$full_name', '$national_id', '$email', '$phone', '$case_number', '$ruling_year', '$target_file', '$service_name', '$forgery_score', '$verdict', '$application_date', '$expected_feedback_date')";

        if (mysqli_query($conn, $sql)) {
            header("Location: ../userdashboard.php?msg=Application submitted for forensic verification");
        } else {
